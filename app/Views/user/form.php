        ...
        <form action="<?= isset($user) ? '/user/update/' . esc($user['id_user']) : '/user/create' ?>" method="post" class="w-full p-6 space-y-6">
            ...
            <div class="w-full space-y-4">
                ...
                <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="w-full">
                        <label for="id_anggota_ref" class="block text-sm font-medium text-gray-700 mb-2">
                            Anggota Referensi
                        </label>
                        <select name="id_anggota_ref" 
                                id="id_anggota_ref" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            <option value="">Pilih Anggota (Opsional)</option>
                            <?php if (!empty($anggota_list)): ?>
                                <?php foreach ($anggota_list as $anggota): ?>
                                    <option value="<?= esc($anggota['id_anggota']) ?>" <?= old('id_anggota_ref', $user['id_anggota_ref'] ?? '') == $anggota['id_anggota'] ? 'selected' : '' ?>>
                                        <?= esc($anggota['nama_lengkap']) ?> - <?= esc($anggota['no_anggota']) ?>
                                    </option>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </select>
                        <?php if (session('errors.id_anggota_ref')): ?>
                            <p class="text-red-600 text-sm mt-1 flex items-center gap-1">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <?= session('errors.id_anggota_ref') ?>
                            </p>
                        <?php endif; ?>
                    </div>
                </div>
